...
// Day 1
int getBit(long lst[], int pos){
    return (lst[pos/32] & (1 << (pos%32) )) != 0;
}

void setBit(long lst[], int pos){
    lst[pos/32] |= 1 << (pos%32);
}


int day1p1(int *nums, int size){
    long flags[64] = {0};
    for (int i = 0; i < size; i++){
        if (getBit(flags, 2020 - nums[i])) {
            return (2020 - nums[i]) * nums[i];
        }
        else{
            setBit(flags, nums[i]);
        }
...
int day2p1(struct passdata *data, int size){
    int result = 0;
    int numchars;
    for (int index = 0; index < size; index++) {
        numchars = 0;
        for (int charindex = 0; charindex < 20; charindex++){
            if (data[index].password[charindex] == data[index].character) { numchars++;}
        }
        if (numchars >= data[index].first && numchars <= data[index].second) {result++;}
    }
    return result;
}

int day2p2(struct passdata *data, int size) {
    int result = 0;
    char first;
    char second;
    for (int index = 0; index < size; index++){
        first = data[index].password[data[index].first - 1];
        second = data[index].password[data[index].second - 1];
        if ((first == data[index].character || second == data[index].character) && (first != second)) {result++;}
    }
    return result;
}
void dayTwo(void) {
    struct passdata data[] = {{1,5,'k',"kkkkhkkkkkkkkkk"},
    ...
    struct passdata testdata[] = {{1,3,'a', "abcde"}}; //, {1,3,'b', "cdefg"}, {2,9,'c', "ccccccccc"}};
    printf("\nPart 1: %d", day2p1(data, 999));
    printf("\nPart 2: %d", day2p2(data, 999));
}
